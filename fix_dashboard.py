...
old_dash = r"\{activeTab === 'dashboard' && \([\s\S]*?<\/>\s*\)\s*}\s*<\/div>\s*\)}"
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {dashboardData.sla_risks.map((r: any, i: number) => (
                            <tr key={i} className="hover:bg-slate-50 cursor-pointer" onClick={() => handleInvestigate(`Investigate ${r.ticket_id} for SLA risk.`)}>
                              <td className="px-4 py-3 font-medium text-blue-600 text-xs">{r.ticket_id}</td>
...
# Remove huge empty vertical spacing in Lists pages
content = content.replace('<div className="flex justify-between items-center mb-6">', '<div className="flex justify-between items-center mb-4 border-b border-slate-200 pb-4">')

# Make ID clickable using blue
content = content.replace('<td className="px-4 py-3 font-medium text-blue-600">', '<td className="px-4 py-3 font-semibold text-blue-600 hover:underline">')
